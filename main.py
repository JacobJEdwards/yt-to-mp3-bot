...
async def sendURL(update: Update, context: CallbackContext) -> None:
    await update.message.reply_text(
        "Send a Youtube Video URL to convert to an MP3 file:"
    )
...
async def getMP3(update: Update, context: CallbackContext) -> None:
    ...
    if not await checkURL(url):
        await update.message.reply_text(
            "Sorry, this is not a valid YouTube video link.\nPlease send a valid link, "
            "use /help for support or contact me @JacobJEdwards"
        )
        return

    message = await context.bot.send_message(
        chat_id=userID, text="_fetching video data..._", parse_mode="Markdown"
    )
    ...
    try:
        ...
        await context.bot.edit_message_text(
            chat_id=userID,
            message_id=messageID,
            text="_Sending file..._",
            parse_mode="Markdown",
        )
        await context.bot.send_document(
            chat_id=userID,
            document=open(zipfilename, "rb"),
            pool_timeout=5,
            connect_timeout=30,
            read_timeout=30,
            write_timeout=45,
        ...
        await context.bot.edit_message_text(
            chat_id=userID, message_id=messageID, text="MP3 File Zipped:"
        )
        ...
        logger.error(
            "unexpected error while running handler callback: %s", str(e), exc_info=True
        )
        await context.bot.edit_message_text(
            chat_id=userID,
            message_id=messageID,
...
def main() -> None:
    ...
    application.add_handler(
        MessageHandler(filters.Regex("YouTube Video to MP3"), sendURL)
    )
